<template>
	...
	<div class="page-table scrollable">
		<h1 class="animated fadeInDown">Awaiting Information</h1>
		<el-row id="space">
...
<script>
	...
	export default {
		name: "life_claims_awaitng_information",
		data() {
			return {
				tableData:[],
				message: '',
		...
		methods: {
			...
			getClaims(){
				let url = this.url+'/lifeclaimsawaitinginfo';
			...
			handleReview(index, row) {
				this.form = row;
				this.id = this.form.life_claim_id;
				this.$router.push('/lifeclaims/review/' + this.id);
			},
