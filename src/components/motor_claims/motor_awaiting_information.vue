<template>
	...
	<div class="page-table scrollable">
		<h1 class="animated fadeInDown" >Awaiting Information</h1>        <el-row id="space">
			...
			<el-table :data="tableData" height="475" style="width: 100%">
				...
				<el-table-column fixed="right" label="Operations" width="250">
					...
					<template slot-scope="scope">
						...
						<el-button type="success" size="mini" slot="reference" @click="handleReview(scope.$index, scope.row)"><span>Review</span></el-button>
...
<script>
	...
	export default {
		name:  "motor_awaititng_information",
		data() {
			return {
				tableData:[],
				message: '',
		...
		methods: {
			...
			getClaims(){
				let url = this.url+'/motorclaimsawaitinginfo';
			...
			handleDelete(index, row) {
				let url = this.url+'/motorclaims/' + row.motor_claim_id;
			...
			handleEdit(index,row){
				this.form=row;
				this.id=this.form.motor_claim_id;
				this.$router.push('/motorclaims/edit/'+this.id);
			},
			handleReview(index, row) {
				this.form = row;
				this.id = this.form.motor_claim_id;
				this.$router.push('/motorclaims/review/' + this.id);
			},

			exportXlsx(){
				var motorClaimsIdWs = XLSX.utils.json_to_sheet(this.excelForm);
				// Workbook
				var motorClaimsWb = XLSX.utils.book_new();
				//Add worksheet to workbook
				XLSX.utils.book_append_sheet(motorClaimsWb,motorClaimsIdWs);
				XLSX.writeFile(motorClaimsWb, 'Motor-Claims.xlsx');
			},

			addClicked(){
				this.$router.push('motorclaims/add');
			},
			...
			onSubmit() {
				let url = this.url+'/motorclaims/' + this.form.motor_claim_id;
