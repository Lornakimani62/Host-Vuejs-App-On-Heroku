        ...
        data(){
            return  {
                ...
                rules: {
                    ...
                    // ],
                    // supporting_doc2: [
                    //     {required: true, message: 'Please input a town code', trigger: 'blur'}
                    // ],
                    // supporting_doc3: [
                    //     {required: true, message: 'Please input Next of Kin name', trigger: 'blur'}
                    // ],
                    // supporting_doc4: [
        ...
        methods: {
            ...
            uploadXlsx(){
                if (this.xlsx != ''){
                    ...
                    let url = this.url + '/motorclaims/excel';
                    this.axios.post(url, formData, {headers: {'Content-Type': 'application/json',
                            'Authorization': 'Bearer '+this.token
                        }}).then((response)=> {
                        if (response.status == 200){
            ...
            exportXlsx(){
                var claimsIdWs = XLSX.utils.json_to_sheet(this.excelForm);

                // Workbook
                var claimsWb = XLSX.utils.book_new();
                //Add worksheet to workbook

                XLSX.utils.book_append_sheet(claimsWb,claimsIdWs);
                XLSX.writeFile(claimsWb, 'Claims.xlsx');
